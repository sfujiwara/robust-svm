...
# Logging
logging.basicConfig(
    filename="logs/expt_mnist_labelflip.log",
    level=logging.DEBUG,
    filemode="w",
...
sys.stdout = open("logs/mnist_labelflip_stdout.txt", "w")
sys.stderr = open("logs/mnist_labelflip_stderror.txt", "w")
...
outlier_ratio = np.array([0., 0.03, 0.05, 0.1, 0.2, 0.3])
...
# Save as csv
df_ersvm.to_csv("results/mnist/ersvm_labelflip.csv", index=False)
df_enusvm.to_csv("results/mnist/enusvm_labelflip.csv", index=False)
df_var.to_csv("results/mnist/var_labelflip.csv", index=False)
df_ramp.to_csv("results/mnist/ramp_labelflip.csv", index=False)
