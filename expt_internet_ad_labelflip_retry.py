...
"""
ER-SVM の accuracy の判定が f(x, y) > epsilon と少し厳し目になっていたので
epsilon = 0 と変更した実験
"""
...
# Logging
logging.basicConfig(
    filename="logs/expt_internet_ad_labelflip_retry.log",
    level=logging.DEBUG,
    filemode="w",
...
sys.stdout = open("logs/internet_ad_labelflip_retry_stdout.txt", "w")
sys.stderr = open("logs/internet_ad_labelflip_retry_stderror.txt", "w")
...
# Save as csv
df_ersvm.to_csv("results/internet_ad/labelflip/ersvm_retry.csv", index=False)
